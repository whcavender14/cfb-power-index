# =============================================================================
# R/publish/rankings_graphic.R — shareable top-30 PNG (presentation only).
#
# Extracted unchanged from the old run_2026_rankings.R, except that the team
# directory path comes from config/paths.R and a bug that referenced a global
# `png_dir` was fixed. Logos are downloaded once into <out_dir>/team_logos and
# never enter the model. Requires ggplot2, ggimage and magick (+ internet on
# first run). Called by scripts/01_build_ratings.R when CFB_CREATE_GRAPHIC=true.
# =============================================================================
...
create_rankings_graphic <- function(ranking, as_of, out_dir, n = 30L) {
  ...
  team_directory_file <- PATHS$team_directory
  ...
  # Fixed: the original computed this path twice, the second time from a
  # global `png_dir` variable that only existed in the calling script.
  date_clean <- format(as.Date(as_of, tz = "UTC"), "%Y-%m-%d")
