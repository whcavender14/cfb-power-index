...
function App() {
  const [tab, setTab] = useState<Tab>(location.hash === '#betting' ? 'betting' : location.hash === '#simulations' ? 'simulations' : 'ratings')
  ...
  useEffect(() => { const change = () => { if (location.hash === '#simulations') setTab('simulations'); else if (location.hash === '#ratings') setTab('ratings'); else if (location.hash === '#betting') setTab('betting') }; window.addEventListener('hashchange', change); return () => window.removeEventListener('hashchange', change) }, [])
  ...
  const data = tab === 'simulations' ? simulations : ratings
