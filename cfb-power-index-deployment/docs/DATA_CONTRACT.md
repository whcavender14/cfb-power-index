...
The exporter joins by team ID, using exact metadata names and supplied aliases only when an input lacks IDs. It rejects duplicate resolved IDs, wrong seasons, nonfinite numeric values, and probabilities outside `[0,1]`. No fuzzy matching, sportsbook scraping, credential reading, or model fitting occurs during export.
